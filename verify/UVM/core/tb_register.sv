...
class reg_item extends uvm_sequence_item;
	// Input
	rand bit [4:0]	r1_addr_i	;
	rand bit [4:0]	r2_addr_i	;
	rand bit [4:0]	w_addr_i	;
	rand bit [31:0]	w_data_i		;
	rand bit w_ena_i		;
	// Output
	bit [31:0] 	r1_data_o	;
	bit [31:0]	r2_data_o	;
	...
	`uvm_field_int(r1_addr_i,	UVM_ALL_ON | UVM_HEX)	
	`uvm_field_int(r2_addr_i,	UVM_ALL_ON | UVM_HEX)
	`uvm_field_int(w_addr_i,    	UVM_ALL_ON | UVM_HEX)
	`uvm_field_int(w_data_i,    	UVM_ALL_ON | UVM_BIN)
	...
	task run_phase(uvm_phase phase);
		forever begin 
			...
			vif.rst_i         <= 0; // Đảm bảo không reset khi đang chạy
        		...
        		vif.w_addr_i   <= req.w_addr_i;
        		vif.w_data_i        <= req.w_data_i;
        
       			vif.r1_addr_i  <= req.r1_addr_i;
        		vif.r2_addr_i  <= req.r2_addr_i;
			@(posedge vif.clk_i);
			// notice done
...
class reg_monitor extends uvm_monitor;
    ...
    forever begin
        ...
        item.w_addr_i  = vif.w_addr_i;
        item.w_data_i       = vif.w_data_i;
        item.r1_addr_i = vif.r1_addr_i;
        item.r2_addr_i = vif.r2_addr_i;
...
class reg_scoreboard extends uvm_scoreboard;
    ...
    function void write(reg_item item);
        ...
        if (item.w_ena_i == 1 && item.w_addr_i != 0) begin
            
            // Cập nhật giá trị vào mảng tham chiếu
            ref_regs[item.w_addr_i] = item.w_data_i;

            `uvm_info("SCB", $sformatf("Update Model: Wrote %h into Reg[%0d]", 
                                       item.w_data_i, item.w_addr_i), UVM_HIGH)
        end
        // Lưu ý: Nếu ghi vào địa chỉ 0, ta KHÔNG làm gì cả (để ref_regs[0] luôn giữ là 0)

        if (item.r1_data_o == exp_r1_data) begin
        `uvm_info("SCB", $sformatf("PASS! Addr:%0h | Exp:%0h | Act:%0h", item.r1_addr_i, exp_r1_data, item.r1_data_o), UVM_LOW)
    end else begin
        `uvm_error("SCB", $sformatf("FAIL! Addr:%0h | Exp:%0h | Act:%0h", item.r1_addr_i, exp_r1_data, item.r1_data_o))
    end
    endfunction
...
endclass
    ...
    class reg_basic_test extends uvm_test;
        ...
        task run_phase(uvm_phase phase);
            ...
            repeat(500000) begin
                item = reg_item::type_id::create("item");
                item.r1_addr_i = $urandom_range(0, 31);
		item.r2_addr_i = $urandom_range(0, 31);
		item.w_ena_i = $urandom_range(0, 1);
		item.w_addr_i = $urandom_range(0, 31);
		item.w_data_i = $urandom();
        `uvm_info("TEST", $sformatf("Sending item: W_Addr=%0h, w_data_i=%0h", item.w_addr_i, item.w_data_i), UVM_LOW)
...
`include "uvm_macros.svh"
    ...
    register  dut (// 1. Clock & Reset
        ...
        .w_ena_i        (vif.w_ena_i),
        .w_addr_i    (vif.w_addr_i),
        .w_data_i       (vif.w_data_i),    // RTL thường có đuôi _i, Interface của bạn đặt là w_data_i

        // 3. Read Port 1 (Cổng đọc 1)
        .r1_addr_i   (vif.r1_addr_i),
       	.r1_data_o      (vif.r1_data_o),

        // 4. Read Port 2 (Cổng đọc 2)
        .r2_addr_i   (vif.r2_addr_i),
...
initial begin
        ...
        $display("----------------------------------------------------------------------------------------------------");
        $display("Time  | Rst | WE | W_Addr |   w_data_i   || R1_Addr |   R1_Data  || R2_Addr |   R2_Data  ");
        $display("------+-----+----+--------+------------++---------+------------++---------+------------");
        ...
        $monitor("%5t |  %b  |  %b |   %2d   |  %h  ||   %2d    |  %h  ||   %2d    |  %h",
                 ...
                 vif.w_addr_i,  // Cổng Ghi
                 vif.w_data_i,
                 
                 vif.r1_addr_i, // Cổng Đọc 1
                 vif.r1_data_o,
                 
                 vif.r2_addr_i, // Cổng Đọc 2
                 vif.r2_data_o
                 );
    end
    endmodule
